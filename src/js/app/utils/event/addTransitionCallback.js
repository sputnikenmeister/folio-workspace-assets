...
//		d = _.extend(d, {level: "warn", method: "[timeout]", timeoutId: timeoutId, elapsed: timeout, prop: prop});
		execute(true);
	}, timeout);

	listener = function(ev) {
		if (prop == ev.propertyName) {
//			console.debug(ev.target, ev.originalTarget, ev);
//			d = _.extend(d, {method: "[event]", type: ev.type, prop: ev.propertyName});
			execute(true);
		}
//		else {
